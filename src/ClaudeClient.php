...
final class ClaudeClient implements SuggestionSource
{
    private const MAX_TOKENS = 1024;

    private SystemContextProvider $systemContextProvider;
    private ?int $lastOutputTokens = null;
    ...
    public function __construct(
        ...
        $version = trim($anthropicVersion);
        $this->anthropicVersion = $version === '' ? '2023-06-01' : $version;
    ...
    public function suggest(string $prompt, int $count): SuggestionResponse
    {
        ...
        $content = $this->sendMessage($instruction);
        $decoded = $this->decodeJson($content, 'suggestions');

        if (!isset($decoded['suggestions']) || !is_array($decoded['suggestions'])) {
            throw new ClaudeClientException('LLM response missing "suggestions" array.');
        }
        ...
        if ($suggestions === []) {
            throw new ClaudeClientException('No usable suggestions were returned by the LLM.');
        }
    ...
    public function listAvailableModels(): array
    {
        ...
        if (!isset($response['data']) || !is_array($response['data'])) {
            throw new ClaudeClientException('Unexpected payload while listing Claude models.');
        }
        ...
        if ($models === []) {
            throw new ClaudeClientException('Claude did not report any models.');
        }
    ...
    private function sendMessage(string $prompt): string
    {
        $payload = [
            'model' => $this->model,
        ...
        $response = $this->post('/v1/messages', $payload);
        $this->recordUsage($response, microtime(true) - $startedAt);

        if (!isset($response['content']) || !is_array($response['content'])) {
            throw new ClaudeClientException('Claude response was missing content.');
        }
        ...
        foreach ($response['content'] as $part) {
            ...
            $type = isset($part['type']) ? strtolower((string) $part['type']) : '';
        ...
        if ($segments === []) {
            throw new ClaudeClientException('Claude response did not include any text segments.');
        }
    ...
    private function recordUsage(array $response, float $duration): void
    {
        ...
        $tokens = null;
        if (isset($usage['output_tokens']) && is_numeric($usage['output_tokens'])) {
            $tokens = (int) $usage['output_tokens'];
        } elseif (isset($usage['completion_tokens']) && is_numeric($usage['completion_tokens'])) {
    ...
    private function request(string $method, string $path, ?string $body): array
    {
        if (!function_exists('curl_init')) {
            throw new ClaudeClientException(
                'The cURL extension is required to contact Claude. Please enable the "curl" PHP extension.'
            );
        }
        ...
        $ch = curl_init($url);
        if ($ch === false) {
            throw new ClaudeClientException('Unable to initialize cURL.');
        }
        ...
        if ($result === false) {
            ...
            throw new ClaudeClientException('cURL error while talking to Claude: ' . $error);
        }
        ...
        if ($status >= 400) {
            throw new ClaudeClientException(sprintf('Claude returned HTTP %d: %s', $status, $result));
        }

        $decoded = json_decode($result, true);
        if (!is_array($decoded)) {
            throw new ClaudeClientException('Failed to decode Claude response: ' . $result);
        }
    ...
    private function resolveApiKey(): string
    {
        $candidates = [
            ...
            getenv('CLAUDE_API_KEY') ?: null,
            $_SERVER['ANTHROPIC_API_KEY'] ?? null,
            $_ENV['ANTHROPIC_API_KEY'] ?? null,
            $_SERVER['CLAUDE_API_KEY'] ?? null,
            $_ENV['CLAUDE_API_KEY'] ?? null,
        ];
        ...
        throw new ClaudeClientException(
            'Missing Claude API key. Set claude.api_key in ~/.shsuggest or the ANTHROPIC_API_KEY environment variable.'
        );
    }
...
PROMPT;
    ...
    private function decodeJson(string $raw, string $expectedKey): array
    {
        ...
        throw new ClaudeClientException(sprintf(
